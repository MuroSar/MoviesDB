...
class MovieService {
    ...
    fun getUpcomingMovies(page: Int): Result<MoviePage> {
        val callResponse = api.createService(MovieApi::class.java).getUpcomingMovies(page, API_KEY_V3)
        val response = callResponse.execute()
        if (response != null) {
            if (response.isSuccessful) {
                response.body()?.let { mapper.transform(it) }?.let {
                    it.category = CATEGORY_UPCOMING
                    it.id = "${it.category}-${it.page}"
                    return Result.Success(it)
                }
            }
